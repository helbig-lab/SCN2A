...
if(input.yaml$cluster == 'c') {
  ...
  for(n in 1:(length(inter_subs))){
    ...
    destfile = paste("mod_scn2a_sub",n,".csv", sep = "")
    ...
    simsubR = paste("source('sim_res_mod_auto_source.R');",simR) #Pasting the sourced R script which calculates the sim score
  ...
  for(n in 1:(length(inter_subs))){
    ...
    shname = paste("mod_",n,".sh",sep = "")
    write(shfile,shname)
    system(shname) #manual processing
...
# 
# 
write_csv(sim_score,paste0(input.yaml$sim_dir,"mod_sim_scn2a.csv")
# 
# 
