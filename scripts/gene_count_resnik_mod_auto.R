...
# input.yaml <- yaml::read_yaml("outputs/input.yml")
system(paste0("cp ../../../scripts/pat_100k_draw_npat_res.R ",input.yaml$output_dir,input.yaml$sim_dir,"."))
# setwd(paste0("outputs/",input.yaml$sim_dir))
...
paper_vars <- read_csv(paste0(input.yaml$variant_file)) %>% 
...
sim_score = read_csv(paste0("mod_sim_scn2a.csv")) 
...
names_1 <- gsub("^X","",cor_names)
names_1 <- gsub("\\.","-",names_1)
# cor_names = names(sim_score)
names(sim_score) = names_1
rownames(sim_score) = names_1
###########################################################
...
pat_pairs = all_genes$Freq %>% unique %>% as.list
  ...
  for( i in 1:length(pat_pairs)) {
    n = pat_pairs[i]
    destfile = paste("100k_",n,"mod_scn2a.csv", sep = "")
    ...
    if(!file.exists(destfile)){
      ...
      p100kR = paste0("source('pat_100k_draw_npat_res.R');",ndraw)
      write(p100kR,rname)
...
exact_p_mod2 <- function(sample_size,sim_score_mod,number_random_draws){
  #builds matrix with nx random draws with n random individuals, example SCN1A with 21 pairs
  nx = number_random_draws #might be superfluous, consider deleting
  n = sample_size 
  ax <- as.data.frame(matrix(nrow=nx,ncol=1)) #might be superfluous, consider deleting
  names(ax) <- c("random_draw") #might be superfluous, consider deleting
...
for (i in 1:nrow(gene_count)){
  ...
  e3 = 100000 # 10K unless n=1, then 100K (predefined) #Might be superfluous, consider deleting
...
write.csv(gene_count,paste0("gene_count_mod_scn2a.csv"),row.names = F)
# setwd(input.yaml$output_dir)

message("\n  Resnik similarity analysis - test of statistical significance ran successfully. \n ")
stop = Sys.time()
